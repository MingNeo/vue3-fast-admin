...
describe('useTableList', () => {
  it('can load data', async () => {
    // 创建表单的 ref 对象
    const form = ref({ resetFields: vitest.fn() })
  ...
  it('search success', async () => {
    ...
    await nextTick()
    expect(loading.value).toEqual(true)
    expect(mockService).toHaveBeenCalledWith({ pageNo: 1, pageSize: 10 })
    // 等待请求完成
    await new Promise(resolve => setTimeout(resolve, 10))
    expect(loading.value).toEqual(false)
    ...
    // 验证服务是否使用正确的参数进行调用
    expect(mockService).toHaveBeenCalledWith({ pageNo: 2, pageSize: 10 })
  })
})
